...
매번 수동 실행이 번거로우면, 스크립트 내용을 Vast.ai **onstart 스크립트**에 넣고 **템플릿으로 저장**한다.
그러면 새 인스턴스 생성 시 부팅과 함께 자동으로 메트릭+Tailscale이 뜬다.

```bash
# onstart 예 (auth key는 인자 대신 직접 박거나 vast.ai 환경변수로)
curl -fsSL https://raw.githubusercontent.com/4EVR0/GPU_Serving_Infra/main/setup_gpu.sh \
  | bash -s "$TAILSCALE_AUTH_KEY" vast-gpu-server-2
```

> ⚠️ onstart에는 auth key가 들어가니 **Reusable + Ephemeral** 키를 쓰고, GPU 노드용 ACL/태그를 좁게.
