...
### 콜드스타트 줄이기 — 영구 볼륨 (✅ 2026-07-20 실측 검증됨)
> 측정 근거: `4EVR0-Server/review/2026-07-20-coldstart-volume-verified.md` (+ 07-04 종합/계획)

**콜드스타트의 큰 비용은 "매 destroy마다 날아가는 것을 다시 만드는" 것이다:**
- 가중치 다운로드(`/workspace/models`), torch.compile(`/workspace/.vllm_cache`).
...
**볼륨 1개로 전부 영속화**된다. **같은 볼륨 재부팅 A/B로 검증:**

| 경로 | 무엇 (env) | 1차(빈 볼륨) → 2차(볼륨) |
|---|---|---|
| `/workspace/models` | 가중치 (`--download-dir`) | 다운로드 **30.1s → 0s** ✅ |
| `/workspace/.vllm_cache` | torch.compile 캐시 (`VLLM_CACHE_ROOT`) | compile **45.9s → 12.6s** ✅ (캐시 적중) |
